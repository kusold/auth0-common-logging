...
module.exports.watch = function (logger, server, options) {
  ...
  function createLogEntry(request, log_type) {
    const took = Date.now() - requests_start_time[request.info.id];
    delete requests_start_time[request.info.id];
  ...
  server.events.on({ name: 'request', channels: 'internal' }, (request, data, tags) => {
    if (tags.received && request.path !== '/api/v2/test' &&
      ignorePaths.indexOf(request.path) < 0) {
      requests_start_time[request.info.id] = Date.now();
      return logger.info({
        log_type: 'request',
        req: request
      });
    ...
    .on({ name: 'request', channels: 'error' }, (request, error) => {
      logger.error({
